...
print(get_lenght_of_each_element(my_list));
        ...
        for (int year = 1; year <= years; year++) {
            double population = initialPopulation * Math.pow(1 + growthRate, year - 1);
            double increase = population * growthRate;
            System.out.printf("%5d | %20s | %20s\n", year, df.format(population), df.format(increase));
        }

        int doublePopulationYear = 0;
        for (int year = 1; year <= 100; year++) {
            double population = initialPopulation * Math.pow(1 + growthRate, year);
            if (population >= initialPopulation * 2) {
                doublePopulationYear = year;
                break;
            }
        }
        System.out.println("\nThe population would double in approximately " + doublePopulationYear + " years.");
    }
}
